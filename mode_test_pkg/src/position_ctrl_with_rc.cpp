...
int main(int argc, char **argv)
{
	...
	while(ros::ok()){
		...
		if(status == init_mode){
			...
			if(-0.3 - current_pose.pose.position.z > 0.2){ // if the vehicle altitude is higher than 1.8m, then we think it's time to switching to hover mode...
				printf("error: %f \n",1 - current_pose.pose.position.z);
